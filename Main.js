...
function runFullProjectInitialSetup(passedSpreadsheet) {
  ...
  if (passedSpreadsheet && typeof passedSpreadsheet.getId === 'function') {
    ...
    setupMessages.push(`Using spreadsheet from WebApp: ${activeSS.getName()}.`);
  } else {
    ...
    if (activeSS) {
      setupMessages.push(`Using spreadsheet (fallback): ${activeSS.getName()}.`);
      if (activeSS.getName() !== TARGET_SPREADSHEET_FILENAME) { // TARGET_SPREADSHEET_FILENAME from Config.gs
          try {
              activeSS.rename(TARGET_SPREADSHEET_FILENAME);
              Logger.log(`[${FUNC_NAME} INFO] Renamed fallback sheet to "${TARGET_SPREADSHEET_FILENAME}".`);
              setupMessages.push(`Spreadsheet renamed to "${TARGET_SPREADSHEET_FILENAME}".`);
          } catch (eRename) { Logger.log(`[${FUNC_NAME} WARN] Could not rename fallback sheet: ${eRename.message}`); }
      }
    }
  }
